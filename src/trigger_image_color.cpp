...
//
class TriggerImageColor: public rclcpp::Node
{
public:
  CAMERA_PUBLIC
  explicit TriggerImageColor(const rclcpp::NodeOptions &options)
  : Node("trigger_image_color", rclcpp::NodeOptions(options).use_intra_process_comms(true)),
  // : Node("trigger_image_color", options),
  ...
  camera_info_url_("package://galaxy_camera_u3v/camera_info/mer2_630_60u3c.yaml")
  {
    ...
    parameters_callback_handle_ = this->add_on_set_parameters_callback(std::bind(&TriggerImageColor::on_set_parameters_callback, this, std::placeholders::_1));
    ...
    // publishers
    pub_ = this->create_publisher<sensor_msgs::msg::Image>("image_raw/resized_stream", 10);
    ...
    frame_count_ = 0;
    frame_timer_ = this->create_wall_timer(1s, std::bind(&TriggerImageColor::frame_timer_callback, this));

    param_timer_ = this->create_wall_timer(1s, std::bind(&TriggerImageColor::param_timer_callback, this));

    callback_group_capture_timer_ = create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
    capture_timer_ = this->create_wall_timer(10ms, std::bind(&TriggerImageColor::capture_timer_callback, this), callback_group_capture_timer_);

    // just use for testing - trigger should come from the controller
    // auto frame_duration = 1000000us/FRAME_RATE;
    // trigger_timer_ = this->create_wall_timer(frame_duration, std::bind(&TriggerImageColor::trigger_timer_callback, this));


    capture_trigger_sub_ = this->create_subscription<sensor_msgs::msg::TimeReference>("/capture_trigger", qos, std::bind(&TriggerImageColor::capture_trigger_callback, this, std::placeholders::_1));
...
private:
  ...
  void capture_device(std::string topic, GX_DEV_HANDLE gx_dev_handle, u_char * RGB_image_buf, u_char * image_buf, int64_t color_filter, int64_t payload_size) {
    ...
    if (frame_buffer->nStatus != GX_FRAME_STATUS_SUCCESS) {
      ...
      std::string filename = save_dir + "/" + timestamp_str + ".png";
...
RCLCPP_COMPONENTS_REGISTER_NODE(camera::TriggerImageColor)