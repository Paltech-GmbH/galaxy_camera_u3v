...
//
class TriggerImageMono: public rclcpp::Node
{
public:
  CAMERA_PUBLIC
  explicit TriggerImageMono(const rclcpp::NodeOptions &options)
  : Node("trigger_image_mono", rclcpp::NodeOptions(options).use_intra_process_comms(true)),
  // : Node("trigger_image_mono", options),
  ...
  camera_info_url_("package://galaxy_camera_u3v/camera_info/mer2_630_60u3c.yaml")
  {
    ...
    parameters_callback_handle_ = this->add_on_set_parameters_callback(std::bind(&TriggerImageMono::on_set_parameters_callback, this, std::placeholders::_1));
    ...
    this->declare_parameter<int64_t>("pixel_format", GX_PIXEL_FORMAT_MONO8);
    ...
    this->declare_parameter<std::string>("image_encoding", "MONO8");
    ...
    // this->declare_parameter<int64_t>("acquisition_start", GX_ACQ_MODE_CONTINUOUS);
    ...
    this->declare_parameter<int64_t>("expected_gray_value", 10);
    ...
    this->declare_parameter<int64_t>("acquisition_frame_rate_mode",GX_ACQUISITION_FRAME_RATE_MODE_OFF);
    ...
    // publishers
    pub_ = this->create_publisher<sensor_msgs::msg::Image>("image_raw_mono/resized_stream", 10);
    pub_info_ = this->create_publisher<sensor_msgs::msg::CameraInfo>("camera_info_mono", 10);

    // initialise are start the timer to work out the frames per second)
    auto start_time = std::chrono::steady_clock::now();
    frame_time_ = start_time;
    frame_count_ = 0;
    frame_timer_ = this->create_wall_timer(1s, std::bind(&TriggerImageMono::frame_timer_callback, this));

    param_timer_ = this->create_wall_timer(1s, std::bind(&TriggerImageMono::param_timer_callback, this));

    callback_group_capture_timer_ = create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
    capture_timer_ = this->create_wall_timer(10ms, std::bind(&TriggerImageMono::capture_timer_callback, this), callback_group_capture_timer_);

    // just use for testing - trigger should come from the controller
    // auto frame_duration = 1000000us/FRAME_RATE;
    // trigger_timer_ = this->create_wall_timer(frame_duration, std::bind(&TriggerImageMono::trigger_timer_callback, this));


    capture_trigger_sub_ = this->create_subscription<sensor_msgs::msg::TimeReference>("/capture_trigger", qos, std::bind(&TriggerImageMono::capture_trigger_callback, this, std::placeholders::_1));
    ...
    if (frame_buffer->nStatus != GX_FRAME_STATUS_SUCCESS) {
      ...
      if (image_encoding_ == "RGB8") {
        ...
        memcpy(&msg->data[0],RGB_image_buf,msg_size);
          ...
          // std::cout << "Pixel format: 0x" << std::hex << frame_buffer->nPixelFormat << std::dec << std::endl;
          // Convert 16-bit Bayer to 8-bit grayscale if necessary
          // Raw16toRaw8(frame_buffer, GX_COLOR_FILTER_BAYER_BG, RGB_image_buf);  // reuse buffer
      ...
      std::string save_dir = "/home/umar/test_extraction/image_mono";  // change to your target folder
      std::string filename = save_dir + "/" + timestamp_str + "_mono.png";
      // cv::resize(image, resized, cv::Size(640, 360));  // or any other resolution you want
...
RCLCPP_COMPONENTS_REGISTER_NODE(camera::TriggerImageMono)